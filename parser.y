...
%token<int_value> INT
%token<string_value> ID TYPE

%token SEMI COMMA DOT ASSIGN BITAND BITXOR BITOR LOGICAND LOGICOR NEGATIVE SELFTUNE LB RB LC RC LP RP STRUCT RETURN IF ELSE BREAK CONT FOR READ WRITE
%token SEQ SNE SLT SLE SGT SGE SHL SHR
%token ADD AND BAND BNOT BXOR BOR DIV DIVIDEASSIGN MINUSASSIGN INC DEC MOD MUL MULTIPLYASSIGN NOT OR PLUSASSIGN SUB THEN
